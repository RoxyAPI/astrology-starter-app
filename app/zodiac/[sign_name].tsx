...
const ZodiacSign = () => {
  ...
  const [data, setData] = useState<ZodiacType | null>(null);

  const fetchData = useCallback(async () => {
    const response = await fetch(
      `${process.env.EXPO_PUBLIC_API_URL}/data/astro/astrology/zodiac/${item.name}?token=${process.env.EXPO_PUBLIC_API_KEY}`,
    );
